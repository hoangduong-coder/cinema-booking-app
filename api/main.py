...
@app.post("/add-movie", response_model=list[movie_schema.Movie])
...
def read_all_movie(skip: int = 0, limit: int = 0, db: Session = Depends(get_db)):
    cinema_list = movie.get_cinema(db, skip=skip, limit=limit)
    return cinema_list
...
def read_movie_by_id(cinema_id: int, db: Session = Depends(get_db)):
    cinema_result = movie.get_cinema_by_id(db, cinema_id=cinema_id)
    if cinema_result is None:
        raise HTTPException(status_code=404, detail="Cinema not found")
    return cinema_result


@app.post("/add-cinema", response_model=list[movie_schema.Cinema])
def create_cinema(cinema: movie_schema.CinemaCreate, db: Session = Depends(get_db)):
    return movie.post_cinema(db, new_cinema=cinema)
